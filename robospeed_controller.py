...
CHANGES IN v2.8
─────────────────────────────────────────────────────────────────────
  - Updated for v2.8 GUI: MockDataThread and QTimer now created once
    in MainWindow.__init__ (previously created in _set_run_mode_enabled)
  - win._thread.stop() now reliably works at startup since _thread
    always exists after MainWindow() construction
  - _ui_timer left running — controller relies on it for status updates

CHANGES IN v2.5
─────────────────────────────────────────────────────────────────────
  - Updated for new RightPanel structure (DefectModule-based)
  - get_vision_settings() now reads from mod_cosmetic / mod_led / mod_geometry
  - _on_cycle_done: updates baseline_count and baseline_ready in state
  - _on_loop_finished: sets stopped=True correctly
  - Module states updated live: IDLE → PROCESSING… → COMPLETE
─────────────────────────────────────────────────────────────────────
"""
...
class RoboSpeedController(QObject):
    ...
    def _on_gui_start(self):
        ...
        # Update defect module states to PROCESSING
    ...
    def _on_cycle_done(self, cycle: int, peak: float, force_ok: bool, retract_ok: bool):
        with self._win._lock:
            ...
            # Baseline tracking
            bl_cycles = st.get("baseline_cycles", 30)
            bl_count  = st.get("baseline_count",  0)
            bl_ready  = st.get("baseline_ready",  False)
            if not bl_ready:
                new_bl = bl_count + 1
                st["baseline_count"] = new_bl
                if new_bl >= bl_cycles:
                    st["baseline_ready"] = True
            # Force out of range tracking
            if not force_ok:
